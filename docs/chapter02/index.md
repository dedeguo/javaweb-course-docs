...
纸上得来终觉浅。本章包含**三个递进式实验**，从内存模拟到 AI 辅助开发，再到数据库实战，逐步构建完整的 Web 系统。

### 实验 2-1：Servlet 基础实战（前置）

* **[👉 实验 2-1：基于 Servlet 的用户认证功能实现](lab2-1.md)**
* **任务目标**：使用**内存模拟数据**（不连数据库），掌握 Servlet + Session + JSON 核心用法。
* **核心技能**：Servlet 创建、doPost 请求处理、Session 登录态管理、JSON 解析与响应。

### 实验 2-2：Vibe Coding 实战（AI 辅助）

* **[👉 实验 2-2：Vibe Coding 及 Vibe Learning](lab2-2.md)**
* **任务目标**：使用 **Trae AI IDE**，通过 AI 引导学习 JDBC/连接池，并 AI 辅助开发完整的校园图书管理系统。
* **核心技能**：Agent Skill 使用、AI 辅助编码、MVC 分层架构、借阅业务逻辑、Postman 接口测试。

### 实验 2：综合实战（进阶）

* **[👉 实验 2：核心组件与持久化综合实战](lab2.md)**
* **任务目标**：抛弃内存模拟，直接连接 **MySQL** 数据库，独立完成用户管理 CRUD 与权限拦截。
