...
在课程的第 5 章中，我们需要使用魔塔社区的免费 API 来调用 DeepSeek 或 Qwen 模型。

#### 1. 注册与账号配置（关键）
* **访问官网**：[https://www.modelscope.cn/](https://www.modelscope.cn/)
* **快捷登录**：点击右上角“登录/注册”，推荐使用 **支付宝** 或 **手机号** 快速登录。
* **⚠️ 核心操作：绑定阿里云与实名认证**
    * 鼠标悬停在右上角个人头像，选择 **“账号设置”**。
    * 在“账号基础信息”页中，找到 **“绑定阿里云账号”**。
    * **绑定并实名**：确保已完成阿里云账号的绑定，且状态显示为 **“已实名”**（绿色对勾）。
    * *注意：若未实名，将无法使用 API-Inference 的免费额度。*

#### 2. 获取 Access Token (API Key)
* **进入个人中心**：点击右上角头像 -> 选择 **“个人中心”**。
* **获取令牌**：在左侧菜单找到 **“访问令牌” (Access Token)**。
* **创建 Key**：点击“新建令牌”，复制生成的 Key（通常以 `sk-` 开头）。
* **代码配置**：将此 Key 填入 `application.properties` 的 `spring.ai.openai.api-key` 中。

---

#### 💡 使用限制说明
为了保证调用的稳定性，请留意以下魔塔社区的免费额度限制：

* **总额度**：每位用户每天总计可调用 **2000 次**。
* **单模型限额**：单个模型（如 DeepSeek-V3）每天最高调用 **500 次**。
* **错误处理**：如果遇到 **429 错误**，说明触发了频率限制或额度用尽，请尝试更换模型或次日再试。
* **场景建议**：该 API 仅供教学调试使用，请勿用于高并发或商业化项目。
